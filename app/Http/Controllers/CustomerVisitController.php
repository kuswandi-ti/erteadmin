...
class CustomerVisitController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (auth()->user()->can('customer visit backdate')) {
            $tgl_visit = insertDateToTable($request->tgl_visit);
            $id_sales_person = $request->sales;
        ...
        $sales = SalesPerson::where('id', $id_sales_person)->first();
        if (!$sales) {
            $kode_sales = '-';
            $nama_sales = '-';
        } else {
            $kode_sales = $sales->kode;
            $nama_sales = $sales->nama;
        }

        $toko = Store::where('id', $id_store)->first();
        if (!$toko) {
            $kode_toko = '-';
            $nama_toko = '-';
            $kota_toko = '-';
        ...
        $store = CustomerVisit::create([
            ...
            'id_store' => $id_store,
            'kode_store' => $kode_toko,
            'nama_store' => $nama_toko,
            'kota_store' => $kota_toko,
    ...
    {
        $data['sales'] = SalesPerson::orderBy('nama')->where("id_store", $request->id_toko)->get(["nama", "id"]);
    ...
    public function data(Request $request)
    {
        ...
        return datatables($query)
            ->addIndexColumn()
            ->addColumn('tgl_visit', function ($row) {
                return formatDate($row->tgl_visit);
            })
